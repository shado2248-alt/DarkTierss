...
import { db, usersTable, playersTable, matchesTable, testsTable, playerRatingsTable, tiersTable, tierPromotionsTable } from "@workspace/db";
...
router.get("/admin/stats", async (_req, res): Promise<void> => {
  ...
  const [{ pendingTests }] = await db
    .select({ pendingTests: sql<number>`count(*)::int` })
    .from(testsTable)
    .where(eq(testsTable.status, "pending"));
  const [{ activeStaff }] = await db
    .select({ activeStaff: sql<number>`count(*)::int` })
    .from(usersTable)
    .where(sql`${usersTable.role} IN ('owner','admin','moderator','tester')`);
...
router.get("/admin/analytics", async (req, res): Promise<void> => {
  ...
  // User growth: count users created per day over period
  const userGrowthRaw = await db.execute(
    sql`SELECT DATE(created_at) as date, COUNT(*)::int as value
        FROM users
  ...
  const tierDistRaw = await db.execute(
    sql`SELECT t.name as label, COUNT(pr.id)::int as value
        FROM tiers t
        LEFT JOIN player_ratings pr ON pr.tier_id = t.id
        GROUP BY t.name
        ORDER BY value DESC`
  );

  const gamemodePopRaw = await db.execute(
    sql`SELECT g.name as label, COUNT(m.id)::int as value
        FROM gamemodes g
        LEFT JOIN matches m ON m.gamemode_id = g.id
        GROUP BY g.name
        ORDER BY value DESC`
  );

  res.json({
    userGrowth: (userGrowthRaw.rows as any[]).map((r) => ({
      date: String(r.date),
      value: Number(r.value),
    })),
    matchActivity: (matchActivityRaw.rows as any[]).map((r) => ({
      date: String(r.date),
      value: Number(r.value),
    })),
    tierDistribution: (tierDistRaw.rows as any[]).map((r) => ({
      label: String(r.label),
      value: Number(r.value),
    })),
    gamemodePopularity: (gamemodePopRaw.rows as any[]).map((r) => ({
      label: String(r.label),
      value: Number(r.value),
    })),
  });
...
router.post("/admin/players/:id/reset-rating", async (req, res): Promise<void> => {
  ...
  await db
    ...
    .where(
      and(
        eq(playerRatingsTable.playerId, params.data.id),
        eq(playerRatingsTable.gamemodeId, parsed.data.gamemodeId)
      )
    );
...
router.post("/admin/players/:id/change-tier", async (req, res): Promise<void> => {
  ...
  if (existing) {
    ...
    await db.insert(tierPromotionsTable).values({
      ...
      fromTierId: existing.tierId,
      toTierId: parsed.data.tierId,
    });
